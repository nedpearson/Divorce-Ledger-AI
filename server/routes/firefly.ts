...
import {
  ...
  getFireflyService,
  FireflyAPIError,
} from '../services/firefly-iii.service';
...
router.post('/transactions/from-document', async (req: Request, res: Response) => {
  try {
    ...
    if (!connection) {
      return res
        .status(400)
        .json({ error: 'Firefly III not connected. Please connect to Firefly III first.' });
    }
    ...
    console.log(
      `[Firefly] Created transaction from document ${data.documentId}: ${result.data.id}`
    );
