...
import { databases, DATABASE_ID, Query, ID, Permission, Role } from '../services/appwrite/client';
...
async function authorizeFileAccess(
  req: MulterRequest,
  fileId: string
): Promise<{ authorized: boolean; file: any; userId: string; error?: string }> {
...
router.get('/status', async (req: Request, res: Response) => {
  ...
  if (!configured) {
    return res.json({
      configured: false,
      message:
        'Appwrite not configured. Please set APPWRITE_ENDPOINT, APPWRITE_PROJECT_ID, and APPWRITE_API_KEY',
    });
  }
...
router.post('/files/upload', upload.single('file'), async (req: MulterRequest, res: Response) => {
  try {
    ...
    setImmediate(async () => {
      try {
        console.log(
          `[Appwrite Routes] Auto-triggering analysis for newly uploaded file ${file.$id}`
        );
...
router.get('/files/:id', async (req: MulterRequest, res: Response) => {
  try {
    ...
    if (!authorized) {
      const statusCode =
        error === 'Authentication required' ? 401 : error === 'File not found' ? 404 : 403;
...
router.post('/files/:id/analyze', async (req: MulterRequest, res: Response) => {
  try {
    ...
    if (result.success) {
      ...
      const needsReview =
        normalizedOutput?.needs_user_review ??
        updatedFile?.status === FILE_STATUS.SUGGESTED ??
        true;
      ...
      res.json({
        ...
        suggested_category:
          updatedFile?.suggestedCategory ?? normalizedOutput?.suggested_category ?? 'Uncategorized',
        extracted: {
        ...
        verification: {
          verified_fields: Object.entries(verification).reduce(
            (acc, [field, v]: [string, any]) => {
              acc[field] = {
                ok: v?.ok ?? false,
                reason: v?.reason ?? '',
        ...
        validation: {
          ok:
            !needsReview &&
            (normalizedOutput?.warnings?.length ?? 0) === 0 &&
  ...
  try {
    ...
    try {
      ...
      if (typeof rawAmount !== 'number' || !isFinite(rawAmount)) {
        ...
        console.log(
          `[Appwrite Finalize] Skipping financial entry for ${req.params.id}: amount is not a valid number`
        );
      } else if (rawAmount <= 0) {
        financialEntrySkipped = 'zero_or_negative_amount';
        console.log(
          `[Appwrite Finalize] Skipping financial entry for ${req.params.id}: amount is zero or negative (${rawAmount})`
        );
      } else {
        ...
        if (hasExistingExpense || hasExistingIncome) {
          ...
          console.log(
            `[Appwrite Finalize] Skipping financial entry for ${req.params.id}: entry already exists`
          );
        } else {
          ...
          const description =
            extractedData.summary || file.aiSummary || file.title || file.fileName || 'Document';
          const vendor =
            extractedData.vendor_name || extractedData.payee || extractedData.payer || null;
          ...
          if (isIncomeCategory(category)) {
            ...
            console.log(
              `[Appwrite Finalize] Created income entry ${income.id} for document ${req.params.id}`
            );
          } else {
            ...
            console.log(
              `[Appwrite Finalize] Created expense entry ${expense.id} for document ${req.params.id}`
            );
          }
        }
      }
      ...
      console.error(
        `[Appwrite Finalize] Failed to create financial entry for ${req.params.id}:`,
        financialError
      );
    }
...
router.delete('/files/:id', async (req: MulterRequest, res: Response) => {
  try {
    ...
    if (!deleteResult.success) {
      console.error(
        `[Appwrite Routes] Failed to delete file ${req.params.id}: ${deleteResult.error}`
      );
    ...
    console.log(
      `[Appwrite Routes] File ${req.params.id} fully deleted from Appwrite (storage + database)`
    );
