...
async function runGoldenSetValidation(options: {
  ...
  const hasRealFiles = fs.existsSync(GOLDEN_SET_FILES_DIR) && 
    fs.readdirSync(GOLDEN_SET_FILES_DIR).length > 0;
  ...
  console.log(`Mode: ${options.useMockData ? 'MOCK (framework validation)' : 'REAL (pipeline validation)'}`);
  console.log('');
...
function createMockExtraction(golden: typeof GOLDEN_SET_DOCUMENTS[0]): NormalizedAnalysisOutput {
  ...
  return {
    ...
    extracted: {
      vendor_name: expected.entities?.find(e => e.type === 'vendor' || e.type === 'organization')?.name || null,
      payee: expected.entities?.find(e => e.field === 'payee')?.name || null,
      payer: expected.entities?.find(e => e.field === 'payer' || e.type === 'person')?.name || null,
      ...
      total_amount: expected.amounts?.[0] ? { value: expected.amounts[0].value, currency: 'USD' } : null,
      subtotal: expected.amounts?.find(a => a.field === 'subtotal') ? 
        { value: expected.amounts.find(a => a.field === 'subtotal')!.value, currency: 'USD' } : null,
      tax_amount: expected.amounts?.find(a => a.field === 'tax') ? 
        { value: expected.amounts.find(a => a.field === 'tax')!.value, currency: 'USD' } : null,
      tip_amount: null,
      shipping_amount: null,
      discount_amount: null,
      balance_due: expected.amounts?.find(a => a.field.includes('balance')) ? 
        { value: expected.amounts.find(a => a.field.includes('balance'))!.value, currency: 'USD' } : null,
      ...
      merchant_city: null,
      merchant_state: null,
      line_items: expected.lineItems?.map(item => ({
    ...
    category_candidates: [
      { category: golden.category, score: 0.95 },
    ],
