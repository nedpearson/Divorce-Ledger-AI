...
export type OrchestratorStatus = "ok" | "needs_ocr" | "intake_error" | "schema_error" | "fatal_error";
...
export interface OrchestratorAction {
  type: ActionType;
  payload: RequestOcrPayload | CallIntakeEnginePayload | PersistIntakeResultPayload | PrepareApprovalScreenPayload | LogPayload | Record<string, never>;
}
...
function validateIntakeEngineResult(result: any): { valid: boolean; errors: string[]; warnings: string[] } {
...
export async function processDocumentUploadEvent(event: DocumentUploadEvent): Promise<OrchestratorResponse> {
  ...
  for (const action of response.actions) {
    switch (action.type) {
        ...
        try {
          const result = await analyzeDocumentWithIntake(
            ...
            payload.engine_input.ui_language,
            {
              ...
              known_categories: payload.engine_input.existing_context.known_categories.length > 0 
      ...
      case "PERSIST_INTAKE_RESULT": {
        ...
        console.log(`[PERSIST] Document ${persistedData.file_id} - ${persistedData.doc_type} (${persistedData.approval_state})`);
        break;
      }
