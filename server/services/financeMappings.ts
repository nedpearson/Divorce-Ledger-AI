...
export function mapInternalCategoryToFireflyCategory(
  ...
  return categoryMappings[category.toLowerCase().replace(/[\s-]/g, "_")] 
...
export function mapDocTypeToInternalBucket(docType: string): CoreLedgerBucket {
  ...
  const liabilityTypes = [
    "MORTGAGE_STATEMENT", "LOAN_STATEMENT", "CREDIT_CARD_STATEMENT",
    "MORTGAGE", "LOAN", "CREDIT_CARD"
  ];
  const expenseTypes = [
    "UTILITY_BILL", "RECEIPT", "INVOICE", "GENERIC_FINANCIAL_EXPENSE",
    "PROPERTY_TAX", "INSURANCE_POLICY", "EXPENSE"
  ];
 ...
 */
export function mapCoreBucketToRecordType(bucket: CoreLedgerBucket): "expense" | "income" | "asset" | "debt" {
  switch (bucket) {
    case "INCOME":
      return "income";
    case "ASSET":
