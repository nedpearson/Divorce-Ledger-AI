...
Respond ONLY with the JSON object, no additional text.`;
  ...
  try {
    ...
    const category = DOCUMENT_CATEGORIES.includes(result.category) ? result.category : 'other';
    ...
    if (charged && workspaceId && userId !== undefined) {
      await refundCredits(workspaceId, userId, cost, 'document_classification_failed');
    }
...
Respond ONLY with the JSON object, no additional text.`;
  ...
  try {
    ...
    if (charged && workspaceId && userId !== undefined) {
      await refundCredits(workspaceId, userId, cost, 'violation_classification_failed');
    }
...
Respond ONLY with the JSON object.`;
  ...
  try {
    ...
    const recordType = validTypes.includes(result.recordType) ? result.recordType : 'unknown';

    const validFrequencies = ['monthly', 'weekly', 'biweekly', 'quarterly', 'annually', 'one-time'];
    const frequency = validFrequencies.includes(result.frequency) ? result.frequency : 'monthly';
    ...
    if (charged && workspaceId && userId !== undefined) {
      await refundCredits(workspaceId, userId, cost, 'financial_data_extraction_failed');
    }
