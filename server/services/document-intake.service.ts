...
export const documentIntakeResultSchema = z.object({
  ...
  dates: z.object({
    ...
    transaction_dates: z.array(z.object({
      label: z.string(),
      original: z.string(),
...
export function validateSourceTraceCompleteness(result: DocumentIntakeResult): { valid: boolean; errors: string[]; warnings: string[] } {
  const errors: string[] = [];
  ...
  result.line_items.forEach((item, idx) => {
    ...
    if (item.quantity !== null && !hasExactTrace(quantityPath)) {
      errors.push(`TRACEABILITY VIOLATION: Missing source_trace for '${quantityPath}' with value ${item.quantity}`);
    }
    if (item.unit_price_normalized !== null && !hasExactTrace(unitPricePath)) {
      errors.push(`TRACEABILITY VIOLATION: Missing source_trace for '${unitPricePath}' with value ${item.unit_price_normalized}`);
    }
    if (item.line_total_normalized !== null && !hasExactTrace(lineTotalPath)) {
      errors.push(`TRACEABILITY VIOLATION: Missing source_trace for '${lineTotalPath}' with value ${item.line_total_normalized}`);
    }
  });
  ...
  const dateFields: Array<{ path: string; value: string | null }> = [
    ...
    { path: 'dates.service_period_start_normalized', value: result.dates.service_period_start_normalized },
    { path: 'dates.service_period_end_normalized', value: result.dates.service_period_end_normalized },
  ...
  if (!hasExactTrace('doc_type')) {
    errors.push(`TRACEABILITY VIOLATION: Missing source_trace for 'doc_type' with value ${result.doc_type}`);
  }
  
  // Check all classification fields - REQUIRED
  if (!hasExactTrace('classifications.primary_category')) {
    errors.push(`TRACEABILITY VIOLATION: Missing source_trace for 'classifications.primary_category' with value ${result.classifications.primary_category}`);
  }
  if (!hasExactTrace('classifications.sub_category')) {
    errors.push(`TRACEABILITY VIOLATION: Missing source_trace for 'classifications.sub_category' with value ${result.classifications.sub_category}`);
  }
  ...
  const classificationBooleans: Array<{ path: string; value: boolean | null }> = [
    ...
    { path: 'classifications.is_spousal_support_related', value: result.classifications.is_spousal_support_related }
  ];
  ...
  result.ledger_actions_proposed.forEach((action, idx) => {
    if (action.action_type === "ADD_TRANSACTION" && action.transaction.suggested_amount !== null) {
      ...
      const matches = allAmounts.some(a => Math.abs(a - action.transaction.suggested_amount!) < 0.01);
      if (!matches && allAmounts.length > 0) {
        warnings.push(`Ledger action[${idx}] proposes amount ${action.transaction.suggested_amount} not found in extracted amounts`);
      }
    }
  });
...
export async function analyzeDocumentWithIntake(
  ...
  const inputPayload = {
    ...
    existing_context: existingContext || {
      known_accounts: [],
      known_vendors: [],
      known_categories: ["Housing", "Transportation", "Utilities", "Food", "Insurance", "Child Support", "Alimony", "Legal Fees", "Income", "Debt Payment", "Tax", "Other"],
      user_profile: {
        jurisdiction: "US",
        currency: "USD"
      }
  ...
  try {
    ...
    if (!schemaValidation.success) {
      ...
      result.errors_or_warnings = [
        ...result.errors_or_warnings,
        ...schemaValidation.error.errors.map(e => `Schema violation: ${e.path.join('.')} - ${e.message}`)
      ];
    }
    ...
    return {
      ...
      errors_or_warnings: ["Analysis failed due to an error. Please try again or enter information manually."]
    };
  }
}
...
export function mapCategoryToRecordType(category: string): "income" | "expense" | "asset" | "debt" | "unknown" {
...
export function mapCategoryToLedgerBucket(category: string): LedgerBucket {
  ...
  // INCOME bucket
  if (["income", "salary", "wages", "payroll", "earnings", "bonus", "commission", "rental income", "refund", "reimbursement", "sales"].some(k => categoryLower.includes(k))) {
    return "INCOME";
  }
  
  // COGS bucket (Cost of Goods Sold)
  if (["cost of goods", "cogs", "materials", "inventory cost", "manufacturing"].some(k => categoryLower.includes(k))) {
    return "COGS";
  }
  
  // TAX bucket
  if (["tax", "income tax", "sales tax", "property tax", "payroll tax", "irs", "taxes"].some(k => categoryLower.includes(k))) {
    return "TAX";
  }
  
  // OWNER_EQUITY bucket
  if (["owner's equity", "owner equity", "retained earnings", "owner's draw", "owner draw", "owner contribution"].some(k => categoryLower.includes(k))) {
    return "OWNER_EQUITY";
  }
  
  // TRANSFER bucket
  if (["transfer", "internal transfer", "account transfer"].some(k => categoryLower.includes(k))) {
    return "TRANSFER";
  }
  
  // EXPENSE bucket (most common default for costs)
  if (["housing", "utilities", "transportation", "food", "insurance", "legal fees", "childcare", "education", "entertainment", "rent", "mortgage payment", "advertising", "marketing", "office", "travel", "meals", "subscriptions", "software", "repairs", "maintenance", "shipping", "telephone", "internet", "healthcare", "medical"].some(k => categoryLower.includes(k))) {
    return "EXPENSE";
  }
  
  // ASSET bucket
  if (["real estate", "property", "investment", "savings", "bank account", "vehicle", "jewelry", "art", "equipment", "accounts receivable"].some(k => categoryLower.includes(k))) {
    return "ASSET";
  }
  
  // LIABILITY bucket (replaces old "debt")
  if (["debt", "loan", "credit card", "mortgage", "owed", "accounts payable", "liability"].some(k => categoryLower.includes(k))) {
    return "LIABILITY";
  }
...
export function mapLedgerBucketToRecordType(bucket: LedgerBucket): "income" | "expense" | "asset" | "debt" | "unknown" {
  switch (bucket) {
    case "INCOME":
      return "income";
    case "EXPENSE":
