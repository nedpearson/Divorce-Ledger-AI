...
class ReconciliationService {
  ...
  private async runCountReconciliation(pool: any, job: ReconciliationJob): Promise<ReconciliationResult> {
    ...
    const variancePercent = sourceCount > 0 ? (variance / sourceCount) * 100 : (targetCount > 0 ? 100 : 0);
  ...
  private async runSumReconciliation(pool: any, job: ReconciliationJob): Promise<ReconciliationResult> {
    const [sourceResult, targetResult] = await Promise.all([
      safeQuery(pool, 'reconciliation:sumSource', job.sourceQuery, []),
      safeQuery(pool, 'reconciliation:sumTarget', job.targetQuery, [])
    ]);

    const sourceSum = parseFloat(sourceResult.rows[0]?.total_amount || sourceResult.rows[0]?.total || 0);
    const targetSum = parseFloat(targetResult.rows[0]?.total_amount || targetResult.rows[0]?.total || 0);
    
    const variance = Math.abs(sourceSum - targetSum);
    const variancePercent = sourceSum !== 0 ? (variance / Math.abs(sourceSum)) * 100 : (targetSum !== 0 ? 100 : 0);
    const isWithinTolerance = variancePercent <= job.tolerancePercent;
  ...
  private async runHashReconciliation(pool: any, job: ReconciliationJob): Promise<ReconciliationResult> {
  ...
  private async runDetailedReconciliation(pool: any, job: ReconciliationJob): Promise<ReconciliationResult> {
    const sourceResult = await safeQuery(pool, 'reconciliation:detailedSource', job.sourceQuery, []);
    const targetResult = await safeQuery(pool, 'reconciliation:detailedTarget', job.targetQuery, []);
  ...
  private async saveResult(result: ReconciliationResult, runId: string): Promise<void> {
    ...
    try {
      await safeQuery(
        ...
        [
          result.jobId, runId, result.status, result.sourceCount, result.targetCount,
          result.matchedCount, result.mismatchedCount, result.sourceSum, result.targetSum,
          result.variance, result.variancePercent, JSON.stringify(result.details)
        ]
      );
    } catch (e) {
  ...
  async createCustomJob(job: Omit<ReconciliationJob, 'id'>): Promise<string> {
    ...
    const result = await safeQuery(
      ...
      RETURNING id`,
      [job.jobName, job.sourceSystem, job.targetSystem, job.reconciliationType, 
        job.sourceQuery, job.targetQuery, JSON.stringify(job.matchKeys), job.tolerancePercent]
    );
