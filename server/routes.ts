...
export async function registerRoutes(
  ...
  console.log('   GET /api/quickbooks/auth-url');
  ...
  console.log('   GET  /api/firefly/status');
  console.log('   POST /api/firefly/connect');
  console.log('   POST /api/firefly/disconnect');
  console.log('   POST /api/firefly/sync/expense/:expenseId');
  console.log('   POST /api/firefly/sync/income/:incomeId');
  console.log('   POST /api/firefly/sync/all');
  console.log('   GET  /api/firefly/sync-logs');
  console.log('   GET  /api/firefly/accounts');
  console.log('   POST /api/firefly/transactions/from-document');
  console.log('   GET  /api/firefly/categories');

  // Register Appwrite routes
  app.use('/api/appwrite', appwriteRoutes);
  console.log('📦 Appwrite Integration Routes Loaded');
  console.log('   GET  /api/appwrite/status');
  console.log('   POST /api/appwrite/setup');
  console.log('   POST /api/appwrite/files/upload');
  console.log('   GET  /api/appwrite/files');
  console.log('   GET  /api/appwrite/files/:id');
  console.log('   POST /api/appwrite/files/:id/approve');
  console.log('   POST /api/appwrite/files/:id/retry');
  console.log('   DELETE /api/appwrite/files/:id');
  console.log('   GET  /api/appwrite/categories');
  ...
  console.log('   GET /api/docs/openapi.yaml - OpenAPI spec (YAML)');
  console.log('   GET /api/docs/openapi.json - OpenAPI spec (JSON)');
  console.log('   GET /api/docs/version - API version info');
  console.log('✅ ETL Pipeline Routes Loaded');
  console.log('   GET /api/etl/status');
  console.log('   GET /api/etl/history');
  console.log('   POST /api/etl/run/full');
  ...
  app.get("/api/dashboard/stats", async (req, res) => {
    try {
      ...
      const stats = await storage.getDashboardStats("demo-user", environment);
      res.json(stats);
    } catch (error) {
  ...
  app.get("/api/patterns", async (req, res) => {
    try {
      ...
      const violations = await storage.getViolations("demo-user", environment);
  ...
  app.get("/api/alerts", async (req, res) => {
    try {
      const environment = (req.query.environment as string) || (req.headers["x-environment"] as string) || "demo";
      const alerts = await storage.getAlerts("demo-user", environment);
      res.json(alerts);
    } catch (error) {
  ...
  app.get("/api/calendar-events", async (req, res) => {
    try {
      const environment = (req.query.environment as string) || (req.headers["x-environment"] as string) || "demo";
      const events = await storage.getCalendarEvents("demo-user", environment);
      res.json(events);
    } catch (error) {
  ...
  app.delete("/api/calendar-events/:id", async (req, res) => {
    try {
      const environment = (req.query.environment as string) || (req.headers["x-environment"] as string) || "demo";
      await storage.deleteCalendarEvent(req.params.id, "demo-user", environment);
      res.json({ success: true });
    } catch (error) {
  ...
  app.get("/api/legal-documents", async (req, res) => {
    try {
      const environment = (req.query.environment as string) || (req.headers["x-environment"] as string) || "demo";
      const docs = await storage.getLegalDocuments("demo-user", environment);
      res.json(docs);
    } catch (error) {
  ...
  app.delete("/api/legal-documents/:id", async (req, res) => {
    try {
      const environment = (req.query.environment as string) || (req.headers["x-environment"] as string) || "demo";
      await storage.deleteLegalDocument(req.params.id, "demo-user", environment);
      res.json({ success: true });
    } catch (error) {
  ...
  app.get("/api/child-support-payments", async (req, res) => {
    try {
      const environment = (req.query.environment as string) || (req.headers["x-environment"] as string) || "demo";
      const payments = await storage.getChildSupportPayments("demo-user", environment);
      res.json(payments);
    } catch (error) {
  ...
  app.delete("/api/child-support-payments/:id", async (req, res) => {
    try {
      const environment = (req.query.environment as string) || (req.headers["x-environment"] as string) || "demo";
      await storage.deleteChildSupportPayment(req.params.id, "demo-user", environment);
      res.json({ success: true });
    } catch (error) {
  ...
  app.post("/api/violations", async (req, res) => {
    try {
      ...
      if (!isDraft) {
        await storage.incrementViolationCount("demo-user");
        
        // Increment voice transcription count if used
        if (audioTranscript && audioTranscript.trim()) {
          await storage.incrementVoiceTranscriptionCount("demo-user");
        }
        
        // Increment media upload count if used
        const mediaCount = (mediaUrls?.length || 0);
        if (mediaCount > 0) {
          await storage.incrementMediaUploadCount("demo-user", mediaCount);
        }
      }
  ...
  app.patch("/api/violations/:id/status", async (req, res) => {
    try {
      ...
      const violation = await storage.updateViolationStatus(req.params.id, "demo-user", environment, status);
      if (!violation) {
  ...
  app.delete("/api/violations/:id", async (req, res) => {
    try {
      const environment = (req.query.environment as string) || (req.headers["x-environment"] as string) || "demo";
      await storage.deleteViolation(req.params.id, "demo-user", environment);
      res.json({ success: true });
    } catch (error) {
  ...
  app.get("/api/violations/:id/evidence", async (req, res) => {
    try {
      const environment = (req.query.environment as string) || (req.headers["x-environment"] as string) || "demo";
      const evidenceFiles = await storage.getEvidenceFiles(req.params.id, "demo-user", environment);
  ...
  app.get("/api/cases", async (req, res) => {
    try {
      ...
      const userCases = await storage.getCases("demo-user", environment);
      res.json(userCases);
    } catch (error) {
  ...
  app.post("/api/cases", async (req, res) => {
    try {
      ...
      await storage.incrementCaseCount("demo-user");
  ...
  app.delete("/api/cases/:id", async (req, res) => {
    try {
      const environment = (req.query.environment as string) || "demo";
      await storage.deleteCase(req.params.id, "demo-user", environment);
      await storage.decrementCaseCount("demo-user");
      res.json({ success: true });
    } catch (error) {
