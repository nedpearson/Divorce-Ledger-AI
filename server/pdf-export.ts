...
export function generateCourtFilingPDF(data: ExportData): PDFKit.PDFDocument {
  ...
  doc.moveDown(0.5);
  doc.fontSize(14).font("Helvetica").text("Divorce Ledger - Forensic Case Management", { align: "center" });
  doc.moveDown(2);
  ...
  if (data.environment === "demo") {
    ...
    doc.fontSize(10).fillColor("red").text("DEMO MODE - NOT FOR COURT USE", 350, boxY + 15);
    doc.fillColor("black");
  }
  ...
  allEvents.slice(0, 20).forEach(event => {
    ...
    if (event.amount !== undefined) {
      const amountStr = event.amount > 0 ? `+$${(event.amount / 100).toFixed(2)}` : `-$${(Math.abs(event.amount) / 100).toFixed(2)}`;
...
function generateHash(data: ExportData): string {
  const content = JSON.stringify({
    violations: data.violations.map(v => ({ id: v.id, timestamp: v.timestamp, description: v.description })),
