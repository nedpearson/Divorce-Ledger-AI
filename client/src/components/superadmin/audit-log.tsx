    ...
    <div className="space-y-4">
      <div className="flex flex-wrap gap-3 items-end">
        <div>
          ...
          <Input placeholder="firm.suspend…" value={actionFilter} onChange={e => setActionFilter(e.target.value)} className="h-8 w-40 text-sm" />
        </div>
        <div>
          ...
          <tbody>
            {data?.entries?.map((entry: any) => (
              <tr key={entry.id} className="border-b hover:bg-muted/20 transition-colors">
                <td className="p-2">
                  <span className={`px-1.5 py-0.5 rounded text-xs font-medium ${colorForAction(entry.actionType)}`}>
                    {entry.actionType}
                  </span>
                </td>
                ...
                <td className="p-2 text-muted-foreground">
                  ...
                  <span className="font-mono text-xs truncate max-w-[80px] inline-block">{entry.targetId ?? "—"}</span>
                </td>
