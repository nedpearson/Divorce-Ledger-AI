...
function UsageSection() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const res = await fetch(`/api/superadmin/analytics/usage?window=${window}`, { credentials: "include" });
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-1">
          {["7d", "30d", "90d"].map(w => (
            <Button key={w} size="sm" variant={window === w ? "default" : "outline"} className="h-6 px-2 text-xs" onClick={() => setWindow(w)}>{w}</Button>
          ))}
        </div>
      </div>
      ...
      {/* By action table */}
      {isLoading && <div className="flex items-center gap-2 text-muted-foreground text-sm"><Loader2 className="h-4 w-4 animate-spin" />Loading…</div>}
      ...
      {data?.byWorkspace?.length > 0 && (
        <div className="rounded-lg border overflow-hidden">
          <div className="bg-muted/50 border-b px-2 py-1.5 text-xs font-medium">Top Workspaces by Usage</div>
          <table className="w-full text-sm">
            <thead>
            ...
            <tbody>
              {data.byWorkspace.slice(0, 10).map((row: any) => (
                <tr key={row.workspaceId} className="border-b hover:bg-muted/20">
                  <td className="p-2 text-xs text-muted-foreground">{row.workspaceName ?? row.workspaceId}</td>
...
function ProfitabilitySection() {
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const res = await fetch("/api/superadmin/analytics/profitability", { credentials: "include" });
      if (!res.ok) throw new Error("Failed");
      return res.json();
    },
  });
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-2">
        ...
        <span className="text-xs text-muted-foreground">(light / typical / heavy usage scenarios)</span>
      </div>
      ...
      {summary && (
        <div className="grid grid-cols-3 gap-3">
          {(["light", "typical", "heavy"] as const).map(scenario => {
            ...
            return (
              <div key={scenario} className={`rounded-lg border p-3 ${s?.marginPct > 0.2 ? "bg-green-50 dark:bg-green-900/10 border-green-200" : s?.marginPct > 0 ? "bg-yellow-50 dark:bg-yellow-900/10 border-yellow-200" : "bg-red-50 dark:bg-red-900/10 border-red-200"}`}>
                <div className="text-xs font-medium capitalize mb-1">{scenario}</div>
                <div className="text-lg font-bold">{fmt$(s?.netCents ?? 0)}</div>
                <div className="text-xs text-muted-foreground">{fmtPct(s?.marginPct ?? 0)} margin</div>
              </div>
            );
          })}
        </div>
            ...
            {plans.map((plan: any) => (
              <tr key={plan.id} className="border-b hover:bg-muted/20">
                ...
                {(["light", "typical", "heavy"] as const).map(scenario => {
                  ...
                  const color = margin > 0.3 ? "text-green-600" : margin > 0 ? "text-yellow-600" : "text-red-600";
                  return (
      ...
      {data?.creditCostModel && (
        <div className="text-xs text-muted-foreground bg-muted/30 rounded p-2">
          Cost model: {fmt$(data.creditCostModel.costPerCreditCents)} / credit &middot;
          Light ×{data.creditCostModel.scenarioMultiplier?.light ?? 0.3} &middot;
          Typical ×{data.creditCostModel.scenarioMultiplier?.typical ?? 0.6} &middot;
          Heavy ×{data.creditCostModel.scenarioMultiplier?.heavy ?? 1.0}
        </div>
      )}
    </div>
  );
