...
import { Loader2, Users, Building2, Scale, CreditCard, TrendingUp, AlertCircle, Zap } from "lucide-react";
...
export default function SuperAdminOverview() {
  ...
  if (isLoading) return <div className="flex items-center gap-2 text-muted-foreground"><Loader2 className="h-4 w-4 animate-spin" /> Loading overview…</div>;
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <StatCard title="MRR Estimate"   value={mrr}                         icon={TrendingUp}  sub="Active subscriptions only" />
        <StatCard title="Active Subs"    value={data?.activeSubscriptions ?? 0} icon={CreditCard} />
        <StatCard title="Delinquent"     value={data?.delinquentCount ?? 0}  icon={AlertCircle} sub="past_due" />
        <StatCard title="Credits (30d)"  value={(data?.creditsConsumed30d ?? 0).toLocaleString()} icon={Zap} sub="AI credits consumed" />
      </div>
    </div>
  );
}
