...
export function AICreditsMeter({ used, limit, resetsAt, compact = false }: AICreditsMeterProps) {
  ...
  return (
    <>
      <Card className="cursor-pointer hover:bg-accent/50 transition-colors" onClick={() => setShowBreakdown(true)}>
        ...
        <CardContent className="space-y-3">
          <div className="text-2xl font-bold">
            {remaining.toLocaleString()} <span className="text-sm font-normal text-muted-foreground">remaining</span>
          </div>
          ...
          {showUpgrade && (
            <div className="flex items-start gap-2 p-2 bg-amber-50 dark:bg-amber-950/30 rounded-md text-sm">
              ...
              <div className="flex-1">
                ...
                <Link href="/pricing">
                  <Button size="sm" variant="ghost" className="h-auto p-0 text-amber-700 dark:text-amber-300">
                    Upgrade for more
                  </Button>
                </Link>
              </div>
