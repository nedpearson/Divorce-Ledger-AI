...
import { TrendingUp, TrendingDown, Users, DollarSign, AlertTriangle, ArrowUpRight, ArrowDownRight } from "lucide-react";
...
function MetricCard({ title, value, subtitle, trend, icon: Icon }: { 
  title: string; 
  value: string; 
  subtitle?: string; 
  trend?: number; 
        ...
        <CardContent>
          <ResponsiveContainer width="100%" height={300}>
            <AreaChart data={monthlyData}>
              ...
              />
              <Area type="monotone" dataKey="mrr" stroke="hsl(var(--chart-1))" fill="hsl(var(--chart-1))" fillOpacity={0.3} />
            </AreaChart>
          </ResponsiveContainer>
        </CardContent>
      </Card>
...
function ChurnLTVTab() {
  const { data: churnData, isLoading: churnLoading } = useQuery<{ summary: ChurnSummary; monthlyData: ChurnData[] }>({
          ...
          <CardContent>
            <ResponsiveContainer width="100%" height={250}>
              <LineChart data={monthlyData}>
                ...
                <YAxis tickFormatter={(v) => `${v}%`} className="text-xs" />
                <Tooltip formatter={(value: number) => `${value.toFixed(1)}%`} />
                <Line type="monotone" dataKey="churnRate" stroke="hsl(var(--chart-3))" strokeWidth={2} dot={{ r: 3 }} />
              </LineChart>
            </ResponsiveContainer>
          </CardContent>
        </Card>
          ...
          <CardContent>
            <ResponsiveContainer width="100%" height={250}>
              <AreaChart data={monthlyData}>
                ...
                <Area type="monotone" dataKey="ltv" stroke="hsl(var(--chart-4))" fill="hsl(var(--chart-4))" fillOpacity={0.3} />
              </AreaChart>
            </ResponsiveContainer>
          </CardContent>
        </Card>
          ...
          {atRiskLoading ? (
            ...
            <div className="space-y-3">
              {atRiskUsers.slice(0, 10).map((user) => (
                <div key={user.userId} className="flex items-center justify-between p-3 rounded-lg bg-muted/50" data-testid={`row-at-risk-${user.userId}`}>
                  ...
                  <div className="flex items-center gap-3">
                    <span className="text-sm text-muted-foreground">{user.daysSinceActivity}d inactive</span>
                    <Badge variant={user.riskScore >= 70 ? "destructive" : user.riskScore >= 50 ? "secondary" : "outline"}>
                      {user.riskScore}% risk
                    </Badge>
                  </div>
                </div>
...
function ViolationsTab() {
  ...
  const { patterns, severityDistribution, totalViolations } = data || { patterns: [], severityDistribution: [], totalViolations: 0 };
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <MetricCard title="Total Violations (30d)" value={formatNumber(totalViolations)} icon={AlertTriangle} />
        <MetricCard title="Categories" value={String(patterns.length)} icon={TrendingUp} />
        <MetricCard 
          title="Avg Severity" 
          value={(patterns.reduce((sum, p) => sum + p.avgSeverity, 0) / Math.max(patterns.length, 1)).toFixed(1)} 
          icon={AlertTriangle} 
        />
      </div>
        ...
        <CardContent>
          <div className="space-y-3">
            {patterns.map((pattern) => (
              <div key={pattern.category} className="flex items-center justify-between p-3 rounded-lg bg-muted/50">
                <div className="flex items-center gap-3">
                ...
                <div className="flex items-center gap-4">
                  ...
                  <Badge variant={pattern.avgSeverity >= 7 ? "destructive" : pattern.avgSeverity >= 4 ? "secondary" : "outline"}>
...
function TierMigrationsTab() {
  const { data, isLoading } = useQuery<{ 
    summary: { totalUpgrades: number; totalDowngrades: number; upgradeRate: number; conversionRate: number };
  ...
  const { summary, monthlyData } = data || { summary: { totalUpgrades: 0, totalDowngrades: 0, upgradeRate: 0, conversionRate: 0 }, monthlyData: [] };
  const { cohorts } = cohortData || { cohorts: [] };

  return (
    <div className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <MetricCard title="Total Upgrades" value={String(summary.totalUpgrades)} icon={ArrowUpRight} />
        <MetricCard title="Total Downgrades" value={String(summary.totalDowngrades)} icon={ArrowDownRight} />
        <MetricCard title="Conversion Rate" value={`${summary.conversionRate?.toFixed(1) || 0}%`} icon={TrendingUp} />
        <MetricCard title="Net Migration" value={String(summary.totalUpgrades - summary.totalDowngrades)} icon={Users} />
      </div>
        ...
        <CardContent>
          <ResponsiveContainer width="100%" height={300}>
            <BarChart data={monthlyData}>
              ...
              <Tooltip />
              <Legend />
              <Bar dataKey="upgrades" name="Upgrades" fill="hsl(var(--chart-2))" radius={[4, 4, 0, 0]} />
              <Bar dataKey="downgrades" name="Downgrades" fill="hsl(var(--chart-3))" radius={[4, 4, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </CardContent>
      </Card>
                ...
                {cohorts.slice(0, 6).map((cohort) => (
                  <tr key={cohort.cohort} className="border-b hover-elevate">
                    ...
                    <td className="text-center p-2">
                      <Badge variant={cohort.month1 >= 80 ? "default" : cohort.month1 >= 50 ? "secondary" : "outline"}>
                        {cohort.month1}%
                      </Badge>
                    </td>
                    <td className="text-center p-2">
                      <Badge variant={cohort.month2 >= 70 ? "default" : cohort.month2 >= 40 ? "secondary" : "outline"}>
                        {cohort.month2}%
                      </Badge>
                    </td>
                    <td className="text-center p-2">
                      <Badge variant={cohort.month3 >= 60 ? "default" : cohort.month3 >= 30 ? "secondary" : "outline"}>
                        {cohort.month3}%
                      </Badge>
                    </td>
                    <td className="text-center p-2">
                      <Badge variant={cohort.month6 >= 40 ? "default" : cohort.month6 >= 20 ? "secondary" : "outline"}>
                        {cohort.month6}%
                      </Badge>
                    </td>
                    <td className="text-center p-2">
                      <Badge variant={cohort.month12 >= 30 ? "default" : cohort.month12 >= 10 ? "secondary" : "outline"}>
                        {cohort.month12}%
                      </Badge>
                    </td>
                  </tr>
...
export default function AnalyticsDashboard() {
  ...
  return (
    <div className="container mx-auto p-4 md:p-6 space-y-6 pb-24 md:pb-6" data-testid="page-analytics-dashboard">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold">Business Analytics</h1>
          <p className="text-muted-foreground">Revenue, churn, violations, and tier migration insights</p>
        </div>
      </div>
      ...
      <Tabs defaultValue="revenue" className="w-full">
        <TabsList className="grid w-full grid-cols-4">
          ...
          <TabsTrigger value="churn" data-testid="tab-churn">Churn & LTV</TabsTrigger>
          <TabsTrigger value="violations" data-testid="tab-violations">Violations</TabsTrigger>
          <TabsTrigger value="migrations" data-testid="tab-migrations">Tier Migrations</TabsTrigger>
        </TabsList>
