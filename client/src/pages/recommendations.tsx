  ...
  const processVoiceRecording = async (audioBlob: Blob) => {
    ...
    try {
      ...
      if (result.transcription) {
        ...
        setTitle(
          result.transcription.slice(0, 100) + (result.transcription.length > 100 ? '...' : '')
        );
      }
    } catch (error) {
      toast({
  ...
  const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (result.extractedText) {
        ...
        setTitle(
          result.extractedText.slice(0, 100) + (result.extractedText.length > 100 ? '...' : '')
        );
      }
    } catch (error) {
      toast({
    ...
    <div className="container max-w-4xl py-6 space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Share a feature request, bug report, or improvement suggestion
          </CardDescription>
        </CardHeader>
