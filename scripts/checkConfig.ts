...
function validateModeConsistency(config: z.infer<typeof configSchema>): ConfigWarning[] {
  ...
  if (!appMode) {
    warnings.push({
      ...
      message: 'APP_MODE not set. Defaulting based on NODE_ENV. Set APP_MODE explicitly for clarity.',
      severity: 'warn',
    });
  }
  ...
  if (nodeEnv === 'production' && appMode === 'demo') {
    warnings.push({
      ...
      message: 'NODE_ENV=production but APP_MODE=demo. This is unusual. Verify this is intentional.',
      severity: 'warn',
    });
  }
  ...
  if (nodeEnv === 'development' && appMode === 'live') {
    warnings.push({
      ...
      message: 'NODE_ENV=development but APP_MODE=live. Live mode should run in production environment.',
      severity: 'error',
    });
  }
  ...
  if (nodeEnv === 'production' && !appMode) {
    warnings.push({
      ...
      message: 'NODE_ENV=production requires APP_MODE to be explicitly set (live or demo). This is mandatory for safety.',
      severity: 'error',
    });
  }
...
function checkConfig() {
  ...
  const effectiveMode = result.data.APP_MODE || (result.data.NODE_ENV === 'production' ? 'live' : 'development');
