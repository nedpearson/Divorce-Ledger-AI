...
async function debugExpenses() {
  ...
  try {
    ...
    for (const env of byEnvironment) {
      ...
      console.log(`  Total Amount: $${env.totalAmount ? (Number(env.totalAmount) / 100).toFixed(2) : "0.00"}`);
      console.log("");
    }
    ...
    for (const user of byUserId) {
      ...
      console.log(`  Total Amount: $${user.totalAmount ? (Number(user.totalAmount) / 100).toFixed(2) : "0.00"}`);
      console.log("");
    }
