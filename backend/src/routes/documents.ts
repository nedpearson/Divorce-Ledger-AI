...
export async function documentRoutes(fastify: FastifyInstance) {
   ...
   */
  fastify.post('/documents/:id/classify', {
    onRequest: [fastify.authenticate],
  }, async (request: FastifyRequest<{
    Params: { id: string };
  }>, reply: FastifyReply) => {
    ...
    if (!bodyResult.success) {
      throw new ValidationError('Invalid classification options', { errors: bodyResult.error.errors });
    }
   ...
   */
  fastify.get('/documents/:id/versions', {
    onRequest: [fastify.authenticate],
  }, async (request: FastifyRequest<{
    Params: { id: string };
  }>, reply: FastifyReply) => {
