...
export const matterMembers = pgTable('matter_members', {
  id: varchar('id', { length: 100 })
    .primaryKey()
    .default(sql`gen_random_uuid()::text`),
  matterId: varchar('matter_id', { length: 100 })
    .notNull()
  ...
  permissions: jsonb('permissions')
    .default({ can_view: true, can_upload: false, can_comment: true })
    .notNull()
    .$type<{
      can_view: boolean;
      can_upload: boolean;
...
export const invitations = pgTable('invitations', {
  ...
  workspaceId: varchar('workspace_id', { length: 100 })
    .notNull()
    .references(() => workspaces.id, { onDelete: 'cascade' }),
  matterId: varchar('matter_id', { length: 100 }).references(() => matters.id, {
    onDelete: 'cascade',
  }),
