...
async function uploadDocument(file: File, userId: string, documentId: string) {
  ...
  const { data, error } = await supabase.storage.from('documents_raw').upload(filePath, file, {
    cacheControl: '3600',
    upsert: false,
  });
...
async function getDocumentDownloadUrl(userId: string, documentId: string, storagePath: string) {
...
async function downloadDocument(storagePath: string, originalFilename: string) {
  const { data, error } = await supabase.storage.from('documents_raw').download(storagePath);
...
async function getUserStorageUsage(userId: string) {
  const { data, error } = await supabase.rpc('get_user_storage_usage', { target_user_id: userId });
...
async function cleanupOldExports() {
  const { data: oldFiles } = await supabaseServiceRole.storage.from('exports').list('', {
    limit: 1000,
  ...
  if (filesToDelete.length > 0) {
    await supabaseServiceRole.storage.from('exports').remove(filesToDelete);
  }
...
async function deleteDocumentCompletely(documentId: string, userId: string) {
  ...
  for (const file of filesToDelete) {
    await supabaseServiceRole.storage.from(file.bucket).remove([file.path]);
  }
...
async function generateThumbnail(documentId: string, storagePath: string) {
  ...
  await supabase.from('documents').update({ thumbnail_path: data.path }).eq('id', documentId);
}
```
